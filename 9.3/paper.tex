...
\caption{All reference cells in 2D (triangle, quadrilateral) and 3D (tetrahedron, pyramid, wedge, hexahedron) with the support point indicated both for linear ($\bullet$) and quadratic ($\times$) shape functions.}
...
Many freely available mesh generation tools produce such kind of meshes; while users of \dealii used to have to pre-process such meshes and convert them to pure hex meshes, users
can now directly work with them.

\subsubsection{Refactoring of internal data structures}

To be able to enable simplex and mixed mesh support, we performed a major refactoring of the internal data structures of \dealii. In particular, the \texttt{Triangulation} class and 
...
in the \texttt{GridGenertor} namespace and convert the obtained mesh to a
...
a pure simplex mesh by selecting an appropriate mapping, finite element, and
quadrature class:
...
The right active finite-element index, which points to the right 
finite element for that cell, has to be assigned to each cell:
...
compile-time constant anymore. These informations used to be queried from
...
of a cell, introduced in the last release. This function has been adjusted to be able to deal with
...
potentially in the form of a "scratch data".
...
Until now, \dealii has only supported local smoothing multigrid algorithms~\citep{TODO}. With release 9.3, the support for global coarsening in a geometric and a polynomial sense for continuous (\texttt{FE\_Q}) and discontinuous (\texttt{FE\_DGQ}) elements has been added, which promise less iteration numbers and better parallel scalability with the disadvantage of having to deal with hanging nodes during smoothing and overall more work per iteration.

Global-coarsening algorithms~\citep{TODO} either coarsen each cell or reduce the the polynomial degree of the finite element of each cell and hereby obtains a new system with less unknowns (also know as, "coarse level"). 
...
algorithm. The option that users can set up the transfer operators on their own enables
the mix of polynomial  and geometric coarsening as needed. It should also be noted that a transfer between continuous
and disconinous elements is possible by providing \texttt{DoFHandler}s set up with these kind of elements.

Internally, the transfer operators categorize fine-coarse cells according to type of refinement 
(refinement of no refinement) and the polynomial-degree pair ($k_f$-$k_c$) into groups. 
The projection and restriction is performed on efficient matrix-free kernels applying
1D projection and restriction matrices.


The user is responsible for setting up the levels (including, the operators,
smoothers). Please note that global coarsening can currently only performed between active levels. 
The user can use utility
...
The usage of the new transfer operators, incl. of some of the utility functions, in the context of a hybrid multigrid algorithm (p-MG with AMG as coarse-grid solver) for hp problems is demonstrated in the new tutorial \texttt{step-75} (see also Subsection~\ref{subsec:steps}).
...
they use p-multigrid or hp algorithms) this
meant a long compilation time. 

In the current release, we have improved the \texttt{FEEvaluation} class implementations, which do
not rely on the template parameters \texttt{degree} and \texttt{n\_q\_points} (expressed in code
with "-1" and "0"):
\begin{c++}
...
These classes select at run time for common low and medium polynomial degree/quadrature combinations ($k\le 6$ and $q\in\{ k+1, k+2, \lfloor (3k)/2 \rfloor \}$) efficient precompiled implementations and default to non-templated 
...
the same polynomial degree. This lead to user code, which were hard to read due to complicated jump tables. 
...
can be deduced (in particular in simplex case also the face type). These changes enable
users to write matrix-free code independent of hp is used or not.
...
\texttt{MatrixFree} and \texttt{LinearAlgebra::distributed::\allowbreak Vector} have been extended with 
...
The tutorial \texttt{step-67b} shows the usage of the new MPI-3.0 feature
...
For more details and a successful application in the library \texttt{hyper.deal} see \ref{TODO}.
